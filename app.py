...
# ==============================================================================
# [1] 시스템 포털 엔진 설계 (Deep CSS - 타이틀 크기 고도화 및 카드 완전 제거)
# ==============================================================================
...
st.markdown("""
    ...
    <style>
        ...
        /* 🚨 [타이틀 수정] 메인 타이틀 크기 21px로 확대 (기존 대비 +3pt) */
        .section-header-text {
            font-size: 21px !important;
            font-weight: 800 !important;
            color: #0F172A !important; /* 더 깊은 블랙으로 선명도 강화 */
        ...
        .kpi-label { font-size: 14px; font-weight: 600; color: #64748B; margin-bottom: 6px; }
        .kpi-value { font-size: 40px; font-weight: 800; color: #1E40AF; line-height: 1; letter-spacing: -1px; }
...
if selected_months:
    ...
    if not full_df.empty:
        ...
        if not df_26.empty:
            ...
            r_df = df_26.groupby('하위품목 텍스트')[['이론금액','실제금액']].sum().reset_index()
            r_df['yield'] = (r_df['이론금액'] / r_df['실제금액'] * 100)
            r_count = len(r_df[(r_df['실제금액'] >= 400000000) & (r_df['yield'] <= 98.0)])
            
            st.markdown('<div class="portal-card">', unsafe_allow_html=True)
            k1, k2, k3, k4 = st.columns(4)
            with k1: st.markdown(f'<div class="kpi-tile"><p class="kpi-label">종합 수율</p><div class="kpi-value">{y_v:.2f}<span class="kpi-unit">%</span></div><p class="kpi-trend" style="color:#22C55E;">▲ 정상 가동</p></div>', unsafe_allow_html=True)
            with k2: st.markdown(f'<div class="kpi-tile"><p class="kpi-label">누적 실제 투입</p><div class="kpi-value">{ac_s/100000000:,.1f}<span class="kpi-unit">억</span></div><p class="kpi-trend" style="color:#64748B;">KRW 누적 실적</p></div>', unsafe_allow_html=True)
            with k3: st.markdown(f'<div class="kpi-tile"><p class="kpi-label">고위험 자재</p><div class="kpi-value" style="color:#E74C3C;">{r_count:02d}<span class="kpi-unit">건</span></div><p class="kpi-trend" style="color:#E74C3C;">⚠️ 정밀 점검</p></div>', unsafe_allow_html=True)
            with k4: st.markdown(f'<div class="kpi-tile"><p class="kpi-label">데이터 신뢰도</p><div class="kpi-value" style="color:#1E40AF;">99.9<span class="kpi-unit">%</span></div><p class="kpi-trend" style="color:#1E40AF;">ERP 실시간 동기화</p></div>', unsafe_allow_html=True)
            ...
            with tabs[i]:
                ...
                with c_r:
                    if not t_df.empty:
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"trend_v5_{d_n}")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        cl, cr = st.columns(2)
        with cl:
            m_o = st.selectbox("자재 유형 선택", ["원자재", "부자재", "반제품"], key="mat_filt_v5")
        ...
        with cr:
            # --- [3. 리스크 매트릭스] ---
            st.markdown('<div style="margin-top:-20px;"><p class="section-header-text">🔍 수율 리스크 매트릭스</p></div>', unsafe_allow_html=True)
            r_p = st.selectbox("관제 부서 선택", ["전체 1팀", "면 1과", "면 5과", "스프실"], key="risk_filt_v5")
            ...
            for i, d_n in enumerate(['면 1과', '면 5과']):
                with [tc1, tc2][i]:
                    ...
                    if not d_t.empty:
                        ...
                        st.plotly_chart(fig_t, use_container_width=True, key=f"top_list_v5_{d_n}")
        st.markdown('</div>', unsafe_allow_html=True)
