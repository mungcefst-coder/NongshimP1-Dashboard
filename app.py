...
if selected_months:
    ...
    if not full_df.empty:
        ...
        for i, d_n in enumerate(['면 1과', '면 5과', '스프실', '전체 총합']):
            with tabs[i]:
                c_l, c_r = st.columns([55, 45]) # 표 데이터가 늘어나 가로폭을 좀 더 확보
                t_df = full_df if d_n == '전체 총합' else full_df[full_df['생산부문명'] == d_n]
                
                with c_l:
                    if not t_df.empty:
                        # 🚨 [수정] 표 데이터 심화 가공 로직
                        # 1. 원자재, 부자재, 반제품 기본 데이터
                        ...
                        # 3. 추가 행(원부자재 수율, 전체 수율) 계산
                        additional_rows = []
                        for yr in tmp_pv.index:
                            # 원부자재 수율
                            th_raw_sub = tmp_pv.loc[yr, ('이론금액', '원자재')] + tmp_pv.loc[yr, ('이론금액', '부자재')]
                            ac_raw_sub = tmp_pv.loc[yr, ('실제금액', '원자재')] + tmp_pv.loc[yr, ('실제금액', '부자재')]
                            y_raw_sub = (th_raw_sub / ac_raw_sub * 100) if ac_raw_sub > 0 else 0
                            additional_rows.append({'연도': yr, '자재 유형 내역': '원부자재 수율', '이론금액': th_raw_sub, '실제금액': ac_raw_sub, '수율': y_raw_sub})
                            
                            # 전체 수율
                            th_total = tmp_pv.loc[yr, '이론금액'].sum()
                            ac_total = tmp_pv.loc[yr, '실제금액'].sum()
                            y_total = (th_total / ac_total * 100) if ac_total > 0 else 0
                            additional_rows.append({'연도': yr, '자재 유형 내역': '전체 수율', '이론금액': th_total, '실제금액': ac_total, '수율': y_total})
                ...
                with c_r:
                    if not t_df.empty:
                        ...
                        fig.update_layout(height=230, margin=dict(l=10,r=10,t=10,b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis_title=None, yaxis_title=None, showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.1, xanchor="right", x=1))
                        st.plotly_chart(fig, use_container_width=True, key=f"line_v36_{d_n}")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with cl:
            ...
            m_o = st.selectbox("자재 유형 선택", ["원자재", "부자재", "반제품"], key="mat_filt_v36")
        ...
        with cr:
            ...
            r_p = st.selectbox("관제 부서 선택", ["전체 1팀", "면 1과", "면 5과", "스프실"], key="risk_filt_v36")
            ...
            for i, d_n in enumerate(['면 1과', '면 5과']):
                with [tc1, tc2][i]:
                    ...
                    if not d_t.empty:
                        ...
                        st.plotly_chart(fig_t, use_container_width=True, key=f"top_list_v36_{d_n}")
        st.markdown('</div>', unsafe_allow_html=True)
else:
