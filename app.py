...
st.markdown(f"**현재 조회 데이터:** `{selected_month}` (월별 데이터 격리 가동 중)")
st.markdown("---")

# 2. 개별 월 데이터 로드 및 전처리 함수 (독립 격리)
def preprocess_df(df, month_label):
    ...
    rename_map = {
        ...
        '資材タイプテキスト': '자재 유형 내역', '資재 유형 내역': '자재 유형 내역',
        '品목텍스트': '하위품목 텍스트', '品목 텍스트': '하위품목 텍스트', '品目テキスト': '하위품목 텍스트', '하위품목텍스트': '하위품목 텍스트',
        '理論金額': '이론금액', '實際金額': '실제금액', 'Actual Amount': '실제금액', '实际金额': '실제금액', '실적금액': '실제금액'
    }
    ...
    # 비정상 수율 아웃라이어 제거 (실제 금액이 잡혔는데 수율이 50% 미만인 데이터 제거)
...
def load_all_raw_data(sheet_id):
    """구글 시트의 모든 월별 탭 데이터를 깨끗하게 분리해서 상시 확보"""
    month_data_dict = {}
    for m in ALL_MONTHS:
        try:
...
# 데이터 사전 로드
data_pool = load_all_raw_data(SHEET_ID)

if data_pool:
    # 추이용 전체 데이터 마스터 생성
    trend_raw_df = pd.concat(data_pool.values(), ignore_index=True) if data_pool else pd.DataFrame()
    
    # ⚡ [핵심 수정] 선택한 월에 맞는 데이터만 정확히 매핑하여 격리
    ...
    if not team_df.empty:
        ...
        # 3. 최상단 KPI 대시보드 연산 (독립된 월 데이터 기준)
        ...
        st.warning("⚠️ 선택하신 월에 해당하는 유효한 데이터가 없습니다.")
else:
    st.warning("⚠️ 데이터를 불러올 수 없습니다. 구글 시트의 탭 이름(1월, 2월 등)을 확인해 주세요.")
