...
# 메인 세팅 및 타이틀 버전 수정 (V2.0 구조 혁신 버전)
...
if selected_month:
    ...
    if not team_df.empty:
        ...
        # =========================================================================
        # ⚡ [구조 혁신 1단] 탭을 없애고 과별 비교분석 그래프와 리스크 매트릭스를 좌우 전면 배치
        # =========================================================================
        row1_col1, row1_col2 = st.columns([45, 55]) # 황금 가로 비율 4.5 : 5.5 분할
        ...
        with row1_col2:
            st.subheader("🔍 수율 리스크 매트릭스 (과별 실시간 관제)")
            ...
            if not item_scatter.empty:
                ...
                fig3 = px.scatter(item_scatter, x='실제 투입 금액 (억 원)', y='수율(%)', hover_name='하위품목 텍스트',
                    color='관리 상태', color_discrete_map=scatter_colors, category_orders={'Clarification': ['기준 미달', '기준 달성']})
        ...
        # =========================================================================
        # ⚡ [구조 혁신 2단] 과별 리스크 최상위 집중 관리 대상 Top 5 좌우 듀얼 배치
        # =========================================================================
        ...
        # =========================================================================
        # ⚡ [구조 혁신 3단] 최하단 상세 데이터 테이블 고정 배치 (안정감 있는 마무리)
        # =========================================================================
        st.subheader("📋 과별 상세 수율 현황 표")
