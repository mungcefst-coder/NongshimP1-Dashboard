...
with st.sidebar:
    ...
    selected_months = st.multiselect("분석할 년월(YY.MM) 복수 선택", options=ALL_MONTHS, default=["25.01", "25.02", "25.03", "26.01", "26.02", "26.03"])
...
if selected_months:
    ...
    if not team_df.empty:
                ...
                with tab_col1:
                    ...
                    if not target_df.empty:
                        ...
                        def style_yield_table(styler, threshold_val):
                            ...
                            # ⚡ [오류 수정 핵심 패치] matplotlib가 없는 환경에서도 정상 구동되도록 배경 투명 음영 스타일링으로 변경
                            for col in styler.columns:
                                if '수율' in col:
        ...
        # --- [CARD 4] 핵심 관리 자재 Top 5 ---
        st.markdown('<div class="dashboard-card"><div class="section-header">🚨 핵심 관리 자재 Top 5</div>', unsafe_allow_html=True)
        if "top5_view_mode" not in st.session_state: st.session_state["top5_view_mode"] = "📊 선택한 기간 전체 누적 데이터"
        view_mode = st.session_state["top5_view_mode"]
        target_single_month = st.session_state["top5_single_month_select"] if "top5_single_month_select" in st.session_state else sorted(selected_months)[0]
        
        tab_26, tab_25 = st.tabs(["📅 2026년 실적 분석", "📅 2025년 실적 분석"])
        for target_yr, current_tab in [("26년 누적", tab_26), ("25년 누적", tab_25)]:
            with current_tab:
                yr_df = team_df[team_df['월'] == target_single_month] if view_mode == "🎯 특정 년월 단독 데이터" else team_df[team_df['연도'] == target_yr]
                chart_title_suffix = f"({target_single_month} 단독)" if view_mode == "🎯 특정 년월 단독 데이터" else f"({target_yr[:3]} 선택 기간 누적)"
                if not yr_df.empty:
                    ...
                    for idx, d in enumerate(['면 1과', '면 5과']):
                        with [r3_c1, r3_c2][idx]:
                            ...
                            else: st.caption("해당 기준에 품목 데이터가 존재하지 않습니다.")
                else: st.caption(f"선택한 조건의 {target_yr[:3]} 데이터가 로드되지 않았습니다.")
        ...
        with top5_ctrl_col1:
            st.radio("label_hidden", ["📊 선택한 기간 전체 누적 데이터", "🎯 특정 년월 단독 데이터"], horizontal=True, key="top5_view_mode", label_visibility="collapsed")
        with top5_ctrl_col2:
            if st.session_state["top5_view_mode"] == "🎯 특정 년월 단독 데이터":
                st.selectbox("month_hidden", options=sorted(selected_months), key="top5_single_month_select", label_visibility="collapsed")
            else: st.empty()
        st.markdown('</div>', unsafe_allow_html=True)
else:
