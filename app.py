...
# 실제 ERP 시스템 감성을 위한 CSS 주입 및 네모 박스 유발 요소 제거
st.markdown("""
    ...
    <style>
        ...
        /* [섹션 타이틀] 불필요한 배경색 박스 제거 및 텍스트 강조만 남김 */
        .section-header-text {
            font-size: 18px;
            font-weight: 700;
            color: #1E40AF;
...
with st.sidebar:
    ...
    st.markdown("<p style='opacity:0.6; font-size:12px;'>Portal Version 2.6 (Stable)</p>", unsafe_allow_html=True)
    st.markdown("---")
    selected_months = st.multiselect("📆 분석 대상 년월(YY.MM)", options=ALL_MONTHS, default=["26.01", "26.02", "26.03"])
    search = st.text_input("🔍 품목 실시간 검색")
    st.markdown("---")
    st.info(f"Synchronized: {datetime.now().strftime('%H:%M:%S')}")

# --- [메인 헤더 구역: 불필요한 박스 요소 제거] ---
st.markdown(f"""
...
if selected_months:
    ...
    if not full_df.empty:
        ...
        if not df_26.empty:
            ...
            # 카드 배경을 흰색으로 고정하고 내부 수치만 출력
            st.markdown('<div class="portal-card">', unsafe_allow_html=True)
            k1, k2, k3, k4 = st.columns(4)
            with k1:
                st.markdown(f'<div class="kpi-tile"><p class="kpi-label">종합 수율</p><div class="kpi-value">{y_val:.2f}<span class="kpi-unit">%</span></div><p class="kpi-trend" style="color:#22C55E;">▲ 정상 가동 중</p></div>', unsafe_allow_html=True)
            with k2:
                st.markdown(f'<div class="kpi-tile"><p class="kpi-label">누적 실제 투입</p><div class="kpi-value">{ac_sum/100000000:,.1f}<span class="kpi-unit">억</span></div><p class="kpi-trend" style="color:#64748B;">KRW 누적 실적</p></div>', unsafe_allow_html=True)
            with k3:
        ...
        # --- [CARD 2: 부문별 상세 관제 패널] ---
        # 오렌지색 배경 박스를 제거하고 깔끔한 텍스트 헤더만 사용
        ...
        with col_grid_l:
            ...
            st.markdown('<div class="portal-card">', unsafe_allow_html=True)
            m_opt = st.selectbox("유형 필터", ["원자재", "부자재", "반제품"], key="mat_filt")
        ...
        with col_grid_r:
            ...
            st.markdown('<div class="portal-card">', unsafe_allow_html=True)
            r_dept = st.selectbox("부서 필터", ["전체 1팀", "면 1과", "면 5과", "스프실"], key="risk_filt")
        ...
        st.markdown('<div class="portal-card">', unsafe_allow_html=True)
        t_yr = st.radio("데이터 기준 연도", ["26년", "25년"], horizontal=True)
    ...
    # 데이터 미선택 시 나타나는 경고창 (데이터 선택 시 자동으로 사라짐)
